...
bookmovies_vs_allmovies <- movie_stats %>%
  ...
  arrange(year)%>%
  replace_na(list(number_of_movies=0, total_made_adjusted= 0, average_made_adjusted= 0, percent_book_movies= 0, percent_average_gross= 0, percent_total_gross= 0))
```
...
percent_plot <- map(seq(nrow(bookmovies_vs_allmovies)), ~bookmovies_vs_allmovies[c(seq(.x), rep(.x, nrow(bookmovies_vs_allmovies) - .x)), ]) %>% 
    ...
    geom_smooth() + 
    geom_line(aes(y= percent_book_movies), color= "#4286f4")+
    scale_y_log10()+
    xlab("Year") +
...
gganimate::gganimate(percent_plot, title_frame = FALSE)
